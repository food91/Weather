...
public class Gloading {
    ...
    public interface Adapter {
        ...
        View getView(Holder holder, View convertView, int status,String text,int image);
    }
    ...
    public static class Holder {
        ...
        public void showLoadFailed(String text,int image) {
            showLoadingStatus(STATUS_LOAD_FAILED,text,image);
        }
         ...
         * @see #showEmpty()
         */
        public void showLoadingStatus(int status,String text,int image) {
            if (curState == status || !validate()) {
                return;
            }
            curState = status;
            ...
            try {
                //call customer adapter to get UI for specific status. convertView can be reused
                View view = mAdapter.getView(this, convertView, status,text,image);
                if (view == null) {
                    printLog(mAdapter.getClass().getName() + ".getView returns null");
                    return;
                }
